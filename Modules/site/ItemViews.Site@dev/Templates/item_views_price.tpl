...
<div class="item-views-price">
    ...
    <span class="item-views-price-exact" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
             ...
             {{else}}
                 {{#if showComparePrice}}
                     <div class="item-views-price-old">
                         {{translate 'MSRP:'}}{{comparePriceFormatted}}
                     </div>
                     <div class="middle-price">
                         {{translate 'You Save:$'}}{{savedPrice}}
                     </div>
                     <span class="item-views-price-lead" itemprop="price" data-rate="{{price}}">{{translate 'Now:'}}{{priceFormatted}}</span>
                 {{else}}
                     <span class="item-views-price-lead" itemprop="price" data-rate="{{price}}">{{priceFormatted}}</span>
                 {{/if}}
