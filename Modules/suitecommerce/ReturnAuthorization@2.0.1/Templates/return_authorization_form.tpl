...
<section class="return-authorization-form">
	...
	<form class="return-authorization-form-form">
		<fieldset class="return-authorization-form-items-fieldset">
			{{#if isCreatedFromSalesOrder}}
				<p class="return-authorization-form-items-info">{{translate '<label class="return-authorization-form-items-fieldset-from-label">From: </label><a href="$(0)">Order #$(1)</a>' createdFromURL createdFromOrderNumber}}</p>
				<input type="hidden" name="type" value="salesorder">
			{{else}}
				<p class="return-authorization-form-items-info">{{translate '<label class="return-authorization-form-items-fieldset-from-label">From: </label> <a href="$(0)">Invoice #$(1)</a>' createdFromURL createdFromOrderNumber}}</p>
				<input type="hidden" name="type" value="invoice">
			{{/if}}

			<h5 class="return-authorization-form-products-title">{{translate 'Select products to return'}}</h5>
			<input type="hidden" name="id" value="{{createdFromInternalId}}">
