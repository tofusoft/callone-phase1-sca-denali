	...
	)
{
	...
	,	childViews: {
			'Billing.Address': function ()
				{
					return new AddressDetailsView({
							model: this.profile.get('addresses').get(this.model.get('billaddress'))
						,	hideActions: true
						,	hideDefaults: true
						,	manage: 'billaddress'
					});
		...
		//@method getContext @returns OrderWizard.Module.ShowPayments.Context
	,	getContext: function ()
		{
			var billing_address = this.profile.get('addresses').get(this.model.get('billaddress'))
			...
			return {
				...
				,	showPayments: !!(giftcertificates.length || paymentmethods.length || !this.wizard.hidePayment())
