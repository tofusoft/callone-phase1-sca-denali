	...
	return {
		...
		add: function (view)
		{
			...
			_(view.bindings).each( function(binding, name)
			{
				...
				{
					if (!binding.setOptions || typeof(binding.setOptions.validate) === 'undefined')
					{
						binding.setOptions = binding.setOptions || {};
						binding.setOptions.validate = true;
						binding.setOptions.silent = true;
					}
					if (!binding.events || _(binding.events).indexOf('blur') === -1)
					{
						binding.events = binding.events || [];
						binding.events.push('blur');
					}
				}
			});

			view.clonedModel = new view.model.constructor();
