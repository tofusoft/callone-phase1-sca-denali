...
{{else}}
	{{#if isTrackingNumberCollectionLengthEqual1}}
		...
		{{#if firstTrackingNumberName}}
			<a target="_blank" data-action="tracking-number" href="{{firstTrackingNumberURL}}">{{firstTrackingNumberName}} {{firstTrackingNumberText}}</a>
		{{else}}
			<a target="_blank" data-action="tracking-number" href="{{firstTrackingNumberURL}}">{{firstTrackingNumberText}}</a>
		{{/if}}
	{{else}}
		<div class="order-history-list-tracking-number-control">
			...
			<div class="order-history-list-tracking-number-control-numbers {{#if collapseElements}}collapsed{{/if}}">
				...
				{{#each trackingNumbers}}
					<li>
						<a target="_blank" class="" data-action="tracking-number" href="{{serviceURL}}">{{trackingNumber}}</a>
						{{serviceName}}
					</li>
				{{/each}}
				</ul>
