...
gulp.task('font-awesome', ['sass'], function(cb)
{
	...
	,	dest = path.join(process.gulp_dest, 'font-awesome');
	...
	move_stream.on('end', function()
	{
		...
		var css_stream = gulp.src(path.join(process.gulp_dest, 'css/**/*.css'))
		...
		css_stream.on('end', function()
		{
			xml2js.parseString(fs.readFileSync(fa_svg, {encoding: 'utf8'}).replace(/unicode=\"\&\#x/ig, 'unicode="'), function(err, result)
			{
				...
				ttf_stream.on('end', function()
				{
					...
					{
						var woff_stream = gulp.src([path.join(dest, 'custom', 'fontawesome-webfont.ttf')])
							.pipe(ttf2woff())
							.pipe(gulp.dest(custom_folder))
							.on('end', function()
							{
