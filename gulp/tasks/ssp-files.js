...
function cssNumbersForFile(file)
{
	...
	{
		files = glob.sync(path.join(process.gulp_dest,'css_ie/shopping*.css'));
	}
	else if(file.path.indexOf('my_account') !== -1)
	{
		files = glob.sync(path.join(process.gulp_dest,'css_ie/myaccount*.css'));
	}
	else if(file.path.indexOf('checkout') !== -1)
	{
		files = glob.sync(path.join(process.gulp_dest,'css_ie/checkout*.css'));
	}
